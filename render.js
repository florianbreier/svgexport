...
async function renderSvg(commands, done, stdout) {
  ...
  try {
    ...
    `;
      ...
      const tmpFile = path.join(path.dirname(svgfile), `svgExportOutput-fa5ce2b6d16510-${path.basename(svgfile, ".svg")}.html`);
      await promises.writeFile(tmpFile, renderContent);
...
function Command(input, params, outputfile) {
  ...
  // crop mode
  params.first(/^(pad|meet)$/i, function (_match) {
    output.mode = 'pad';
  });
